...
select f.id, f.name, f.url, f.user_id, f.created_at, f.updated_at, f.last_fetched_at, u.name as user_name from feeds f join users u on f.user_id = u.id
`
...
UPDATE feeds SET last_fetched_at = $2, updated_at = $2 WHERE id = $1 RETURNING id, name, url, user_id, created_at, updated_at, last_fetched_at
`
